    ...
    if(MaxHeightPulseOnly){
      ...
      while (!integrated_rightward && peak_sample!=wmax){
        ...
        if (pulsewinright>=wmax) {integrated_rightward = true;}
         ...
         //std::cout<< "inside Gian 63 " << std::endl;
          ...
          pulsewinright+=1;
            ...
            //std::cout<< "inside Gian 64 " << std::endl;
              raw_area += raw_minibuffer_data.GetSample(pulsewinright);
              charge += sample_height;
              pulsewinright+=1;
  ...
  unsigned short adc_threshold, const unsigned long& channel_key) const
{
  ...
  // "PulseWindowType" default for EventBuilding (to match the 2023 gains calibration integration approach)
      ...
      // loop through samples until we find a pulse, then keep going until we determine the stopping point
      for (size_t s = 0; s < num_samples; ++s) {
        ...
        if ( !in_pulse && raw_minibuffer_data.GetSample(s) > adc_threshold ) {
          ...
          // without advancing the current sample count, we can determine start of pulse by walking back from the threshold crossing point
          // until we dip below baseline, then take 5 samples before as the pulse start
          ...
          while (pulsewinleft > 0) {
            ...
            if (raw_sample_height <= baseline_plus_one_sigma) {
              if (pulsewinleft > 5) {
                ...
                break;
              } else {                                    // if the threshold crossing point < 5 from the beginning of the minibuffer, assign 0 as pulse start
                if(verbosity>v_debug) std::cout << "Pulse is VERY EARLY in the minibuffer (< 5 samples)... assigning pulse start as 0" << std::endl;
        ...
        // or in case we've reached the end of the minibuffer, force pulse to end
          ...
          if (s == num_samples - 1) {
            if(verbosity>v_debug) std::cout << "Pulse is VERY LATE in the minibuffer (we're at the final sample)... forcing pulse to end" << std::endl;
            pulse_end_sample = s;
          } else {
            pulse_end_sample = (s + 5);                   // pulse end = baseline crossing + 5
          }
