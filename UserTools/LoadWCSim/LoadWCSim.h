...
class LoadWCSim: public Tool {
	...
	void LoadMCParticles(WCSimRootTrigger* firstTrig);
	void LoadNeutronCaptures(WCSimRootTrigger* aTrig);
	...
	//TTree* wcsimtree;
	wcsimT* WCSimEntry; 
	WCSimRootTrigger* aTrigTank, *aTrigMRD, *aTrigVeto;
	WCSimRootTrigger* firstTrigTank, *firstTrigMRD, *firstTrigVeto;
	...
	void ConstructDetectors(Geometry* anniegeom, int numDets, std::string system);
	...
	// map to store if a neutron is a primary or secondary,
	// key = neutron track ID, value == true if prim.
	std::map<int, bool>* mapNeutronIsPrim = nullptr;	
	
	// Each MCHit will contain the idx of it's parent MCParticle's
	// position within the MCParticles vector
	//********** Note:- ParentID serves the purpose of the PrimaryParentID in the LoadWCSim tool, I did't want to break other things by just changing the variable name. ********** //
	std::map<int, int>* trackid_to_mcparticleindex = nullptr;
	std::tuple<std::vector<int>, std::vector<int>, bool> GetHitParentIDs(WCSimRootCherenkovDigiHit* digiHit, WCSimRootTrigger* firstTrig);
	...
	// taken from geofile.txt
	std::vector<double> facc_paddle_yorigins =
	  {-198.699875000, -167.999875000, -137.299875000, -106.599875000,
	    -75.899875000,  -45.199875000,  -14.499875000,   16.200125000,
 	     46.900125000,   77.600125000,  108.300125000,  139.000125000,
	    169.700125000, -198.064875000, -167.364875000, -136.664875000,
	   -105.964875000,  -75.264875000,  -44.564875000,  -13.864875000,
	     16.835125000,   47.535125000,   78.235125000,  108.935125000,
	    139.635125000,  170.335125000};
	...
	// additional info
	void MakeParticleToPmtMap(WCSimRootTrigger* thisTrig,
				  WCSimRootTrigger* firstTrig,
				  std::map<int, std::map<unsigned long,double>>* ParticleId_to_DigitIds,
				  std::map<int, double>* ChargeFromParticleId,
